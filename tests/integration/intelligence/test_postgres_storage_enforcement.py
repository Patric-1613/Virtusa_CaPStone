...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_changes_immutability_and_value_survival(open_database_session: _OpenSession) -> None:
    ...
    async with open_database_session() as session:
        ...
        result = await session.scalar(select(ChangeModel.position).where(ChangeModel.id == c_id))
        assert result == 0
...
@pytest.mark.asyncio
async def test_digests_immutability_and_value_survival(open_database_session: _OpenSession) -> None:
    d_id = new_id()
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_extracted_facts_truncate_rejection(open_database_session: _OpenSession) -> None:
    f_id = new_id()
    async with open_database_session() as session:
        _, snap = await _make_source_and_snapshot(session)
        await _ensure_subject(session, company_key="openai", product_key="gpt-4")
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_changeset_change_subject_consistency_rejection(
    open_database_session: _OpenSession,
) -> None:
    async with open_database_session() as session:
        _, snap = await _make_source_and_snapshot(session)
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_published_digest_immutability_and_gate(open_database_session: _OpenSession) -> None:
    ...
    async with open_database_session() as session:
        ...
        with pytest.raises(DBAPIError, match="Cannot update title of an already published digest"):
            await session.execute(
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_publish_idempotency_one_published_per_date(
    open_database_session: _OpenSession,
) -> None:
    d1_id = new_id()
    d2_id = new_id()
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_referential_integrity_and_deletion_restrictions(
    open_database_session: _OpenSession,
) -> None:
    async with open_database_session() as session:
        _, snap = await _make_source_and_snapshot(session)
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_current_facts_composite_foreign_key_consistency(
    open_database_session: _OpenSession,
) -> None:
    async with open_database_session() as session:
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_concurrent_writer_advisory_lock_serialization(
    open_database_session: _OpenSession,
) -> None:
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_source_item_latest_snapshot_composite_fk_rejection(
    open_database_session: _OpenSession,
) -> None:
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_current_facts_atomic_upsert_concurrency_outcomes(
    open_database_session: _OpenSession,
    ...
    # ---------------------------------------------------------------------
    async with open_database_session() as session1:
    ...
    async with open_database_session() as session_verify:
    ...
    # ---------------------------------------------------------------------
    async with open_database_session() as session2:
    ...
    # ---------------------------------------------------------------------
    async with open_database_session() as session3:
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_current_facts_four_part_ordering_tuple_postgres(
    open_database_session: _OpenSession,
) -> None:  # pylint: disable=too-many-locals,too-many-statements,protected-access
    ...
    async with open_database_session() as s1:
        store1 = PostgresFactStore(s1)
    ...
    async with open_database_session() as s_check:
        cf1 = await s_check.get(CurrentFactModel, ("pgtupleco", "modelalpha", "benchmark_scores"))
        assert cf1 is not None
    ...
    async with open_database_session() as s2:
        store2 = PostgresFactStore(s2)
    ...
    async with open_database_session() as s_check:
        cf2 = await s_check.get(CurrentFactModel, ("pgtupleco", "modelbeta", "benchmark_scores"))
        assert cf2 is not None
    ...
    async with open_database_session() as s_tie:
        await _ensure_subject(
            s_tie,
            company_key="pgtieco",
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_current_facts_real_two_connection_concurrency(
    open_database_session: _OpenSession,
    ...
    async with open_database_session() as init_session:
    ...
    async with open_database_session() as verify_session:
        cf_speed = await verify_session.get(CurrentFactModel, ("concurco", "engineasync", "speed"))
...
# -----------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_current_facts_first_write_race_concurrency(
    open_database_session: _OpenSession,
) -> None:  # pylint: disable=too-many-locals
