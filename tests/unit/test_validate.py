...
def test_citation_missing_from_snapshots_by_id_is_unsupported_not_trusted() -> None:
    """Corrected per the third review: a citation id that exists in
    known_snapshot_ids but isn't in snapshots_by_id (e.g. a Change's
    previous-value snapshot from an earlier day's run, outside the
    current batch) can't be content-verified here -- existence of the id
    is NOT treated as proof of support. The claim routes to review
    ...
    historical content (see the design proposal doc) -- an accepted,
    deliberate cost, not a bug."""
